...
UPDATE_FILE = BASE_DIR / 'DellTpm2.0_Fw1.3.2.8_V1_64.exe'
...
DRIVER_32BIT_ADDR = 0x1400571d0
DRIVER_32BIT_SIZE = 0x33f8
DRIVER_64BIT_ADDR = 0x14005a5d0
DRIVER_64BIT_SIZE = 0x39f8
IMAGE_BASE = 0x140000000
DATA_SECTION_RVA = 0x57000
DATA_SECTION_FILE_OFF = 0x55e00
...
with UPDATE_FILE.open('rb') as stream:
    update_data = stream.read()
...
def extract_driver(output_path: Path, virtual_addr: int, size: int):
    ...
    driver_data = bytearray(update_data[file_offset:file_offset + size])
    key = DRIVER_XOR_KEY
    for i in range(size):
...
# Find the firmware using markers
FLASH_PAYLOAD_MARKER_BEGIN = binascii.unhexlify(b'aaeeaa761becbb20f1e651')
FLASH_PAYLOAD_MARKER_END = binascii.unhexlify(b'eeaaee8f491be8ae143790')

marker_offset = 0
while True:
    marker_offset = update_data.index(FLASH_PAYLOAD_MARKER_BEGIN, marker_offset + 4)
    # Find a marker with the right checksum
    if xor_checksum(update_data[marker_offset - 4:marker_offset + 0xc]) == 0:
        payload_size_bytes = update_data[marker_offset - 4:marker_offset]
        payload_size = struct.unpack('<I', payload_size_bytes)[0]
        end_marker = marker_offset + 0xc + payload_size
        if (end_marker + 0x10 < len(update_data) and
                update_data[end_marker:end_marker + 0xf] == payload_size_bytes + FLASH_PAYLOAD_MARKER_END and
                xor_checksum(update_data[end_marker:end_marker + 0x10]) == 0):
            break


print(f"File {UPDATE_FILE}")
print(f"  [{0:06x}..{marker_offset-4:06x}] PE file ({marker_offset-4} bytes")
...
print(f"  [{marker_offset-4:06x}..{marker_offset+0xc:06x}] Flash Payload Start Marker (size + '{xx(FLASH_PAYLOAD_MARKER_BEGIN)}' + byte for checksum)")  # noqa
print(f"  [{marker_offset+0xc:06x}..{end_marker:06x}] Zlib-compressed Flash Payload ({payload_size} bytes)")
print(f"  [{end_marker:06x}..{end_marker+0x10:06x}] Flash Payload End Marker (size + '{xx(FLASH_PAYLOAD_MARKER_END)}' + byte for checksum)")  # noqa
print(f"  [{end_marker+0x10:06x}..{len(update_data):06x}] Authenticode signature")

flash_payload_data: bytes = zlib.decompress(update_data[marker_offset + 0xc:end_marker])
...
if flash_update_tpm2_data:
    # Decode the command packets in the data which is transmitted.
    # Each packet is a TPM 2.0 command, with a structure available for example on
    ...
    # ... Which is the tag for TPM 1.2 commands!
    #
    ...
    while tpm_req_offset < len(flash_update_tpm2_data):
        tpm_req_tag, tpm_req_size, tpm_req_ordinal = \
            struct.unpack('>HII', flash_update_tpm2_data[tpm_req_offset:tpm_req_offset + 0xa])
        ...
        assert tpm_req_offset + tpm_req_size <= len(flash_update_tpm2_data)
        tpm_req_data = flash_update_tpm2_data[tpm_req_offset + 0xa:tpm_req_offset + tpm_req_size]

        # The tag is always TPM_TAG_RQU_COMMAND = 0x00c1
        assert tpm_req_tag == 0x00c1

        if tpm_req_ordinal == 0xaa:
            # First request, TPM 1.2: TPM_ORD_FieldUpgrade = 0x000000AA
            cur_offset = 0
            # There are 2 sub-blocks
            blk1_size = struct.unpack('>I', tpm_req_data[cur_offset:cur_offset + 4])[0]
            assert blk1_size == 0x30
            blk1_data = struct.unpack('>24H', tpm_req_data[cur_offset + 4:cur_offset + 4 + blk1_size])
            # Group by 3 integers (48 = 8 * 6 bytes)
            blk1_data_hex = " ".join(
                f"{blk1_data[idx]:04x}-{blk1_data[idx + 1]:04x}-{blk1_data[idx + 2]:04x}"
                for idx in range(0, len(blk1_data), 3))
        ...
        if tpm_req_ordinal == 0x20000033:
            ...
            try_match_data_with_digests(flash_update_tpm2_data[tpm_req_offset:tpm_req_offset + tpm_req_size])
    ...
    assert tpm_req_offset == len(flash_update_tpm2_data)
